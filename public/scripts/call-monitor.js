...
async function loadClassificationData() {
    try {
        ...
        document.getElementById('classificationPhone').value = currentPendingCall.phone || '';
        document.getElementById('classificationClinicName').value = currentPendingCall.clinic_name || '';

        // If there's an existing classification, load it
        if (currentPendingCall.classification_id) {
            currentClassificationId = currentPendingCall.classification_id;

            const { data: classification, error } = await supabase
                .from('call_classifications')
                .select('*')
                .eq('id', currentPendingCall.classification_id)
                .single();
        ...
        showError('Error loading classification data');
    }
}
...
async function saveClassification() {
    try {
        ...
        if (!selectedType) {
            showError('Please select a classification type');
            return;
        }
        ...
        if (classificationType === 'ivr_only' || classificationType === 'ivr_then_human') {
            ...
            if (ivrActions.length === 0) {
                showError('Please add at least one IVR action');
                return;
            }
        }
        ...
        showInfo(currentClassificationId ? 'Classification updated successfully!' : 'Classification created successfully!');
        closeClassificationModal();

        // Reload call details to show updated classification
        if (currentPendingCall) {
            await loadCallDetails(currentPendingCall.id);
        }
...
// Initialize classification type change handlers on page load
